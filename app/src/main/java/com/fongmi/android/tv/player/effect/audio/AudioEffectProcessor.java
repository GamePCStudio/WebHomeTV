...
public final class AudioEffectProcessor extends BaseAudioProcessor {
    ...
    private static float mixStereoLeft(float[] samples) {
        ...
        float sum = samples[0];
        if (n >= 3) sum += samples[2] * 0.7071f;
        if (n >= 5) sum += samples[4] * 0.7071f;
        if (n >= 7) sum += samples[6] * 0.7071f;
        return sum;
    }

    private static float mixStereoRight(float[] samples) {
        int n = samples.length;
        float sum = n >= 2 ? samples[1] : samples[0];
        if (n >= 3) sum += samples[2] * 0.7071f;
        if (n >= 6) sum += samples[5] * 0.7071f;
        if (n >= 8) sum += samples[7] * 0.7071f;
        return sum;
    }
