...
public abstract class HistoryDao extends BaseDao<History> {
    ...
    @Query("SELECT * FROM History WHERE cid = :cid AND `key` LIKE :prefix || '%' ORDER BY createTime DESC")
    ...
    @Query("SELECT * FROM History WHERE cid = :cid AND vodName LIKE '%' || :keyword || '%' ORDER BY createTime DESC LIMIT 60")
