...
public final class KodiTrueHdEntryAudioSink extends ForwardingAudioSink
   ...
   * inherited ForwardingAudioSink implementation would bypass TrueHD routing by forwarding the
   * whole config straight to the baseline sink, so route here first.
   */
  @Override
  public void configure(AudioSinkConfig audioSinkConfig) throws ConfigurationException {
    AudioSink targetSink = selectSink(audioSinkConfig.format);
    ...
    if (activeSink == baselineSink) {
      ...
      activeSink.configure(audioSinkConfig);
    }
  }
